...
l_freq = 4   # 하한 주파수 (4 Hz)
h_freq = 7   # 상한 주파수 (7 Hz)
...
for epoch in evokeds:
   for evoked in epoch:
       ...
       if marker_id in marker_event_mapping:
           ...
           ers_erd, times = calculate_ers_erd([evoked], baseline_interval, event_interval, theta_band, l_freq, h_freq)
    ...
    for condition in condition_stimulus_ers_erd:
        for stimulus in condition_stimulus_ers_erd[condition]:
            ...
            if ers_erd_results:
                ...
                ax.set_title(f'Mean Theta ERS/ERD - {condition} - {stimulus}')
                fig.colorbar(im)
                ...
                plt.close(fig)
                print(f"Saved mean Theta ERS/ERD topomap for {condition} - {stimulus} to {topomap_save_path}")
                ...
                ax.set_title(f'Theta ERS/ERD Time Series - {condition} - {stimulus}')
