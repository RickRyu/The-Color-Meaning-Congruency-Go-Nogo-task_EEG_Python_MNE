...
# 태스크 조건 설정 (한글로 변경)
TASKS = [
    {"text": "출발", "color": GREEN, "marker_id": 19},
    {"text": "정지", "color": RED, "marker_id": 20},
    {"text": "출발", "color": RED, "marker_id": 21},
    {"text": "정지", "color": GREEN, "marker_id": 22}
]
...
while running:
    ...
    if start_task:
        ...
        if task["marker_id"] == 19 and slash_pressed:
            type_result = "correct"
            correct_count += 1
            if duration is not None:
                correct_response_times.append(duration)
           
        elif task["marker_id"] == 20 and comma_pressed:
            type_result = "correct"
            correct_count += 1
            if duration is not None:
                correct_response_times.append(duration)

        elif task["marker_id"] in [21, 22] and not (slash_pressed or comma_pressed):
            type_result = "correct"
            correct_count += 1
            if duration is not None:
                correct_response_times.append(duration)
